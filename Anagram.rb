...
for example, "PoTS" is an anagram of "sTOp", but StoP is not an anagram of sTOp. The anagram set is the subset of the candidate set that are anagrams 
...
def anagram(target, candidates)
    candidates.select do |candidate|
        candidate.downcase.chars.sort == target.downcase.chars.sort && candidate.downcase != target.downcase
    end
end

# Test cases
puts anagram("stone", ["stone", "tones", "banana", "tons", "notes", "Seton"]) == ["tones", "notes", "Seton"]
