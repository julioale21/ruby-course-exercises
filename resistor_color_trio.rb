...
def get_unit_index(main_value)
    unit_index = 0
    while main_value >= 1000
        main_value /= 1000
        unit_index += 1
    end
    return unit_index
...
def decode_resistance(colors)

    units = ["ohms", "kiloohms", "megaohms", "gigaohms", "teraohms"]
    ...
    main_value = (color_values[colors[0]] * 10 + color_values[colors[1]]) * 10 ** color_values[colors[2]]
    magnitude = get_unit_index(main_value)

    main_value /= 1000.0 ** magnitude

    return "#{main_value.to_i} #{units[magnitude]}"

end

puts decode_resistance([:orange, :orange, :orange])
